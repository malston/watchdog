...
const ConnectionMonitor = () => {
  ...
  // Helper function to parse duration strings (e.g., '1m30s') into seconds
  const parseDuration = (duration: string): number => {
    const regex = /(\d+m)?(\d+s)?/;
    const matches = duration.match(regex);
    const minutes = matches && matches[1] ? parseInt(matches[1]) * 60 : 0;
    const seconds = matches && matches[2] ? parseInt(matches[2]) : 0;
    return minutes + seconds;
  };

  // Mock function to simulate reading log file
  // In a real implementation, this would be an API call to your Go backend
  // Replace the fetchData function in ConnectionMonitor.js
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-2">Internet Connection Monitor</h1>
